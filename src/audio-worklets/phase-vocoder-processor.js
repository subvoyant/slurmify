// ──────────────────────────────────────────────────────────────────────
// src/audio-worklets/phase-vocoder-processor.js — Vendored phaze
// ──────────────────────────────────────────────────────────────────────
//
// Bundled phase-vocoder AudioWorklet from github.com/olvb/phaze.  Used
// in v0.3 as the live-preview pitch shifter (slurmify FX rack — see
// useFxChain.ts).  We switched away from SoundTouchJS WSOLA because
// even at its lowest practical settings (~25 ms sequence + 4 ms
// overlap), the perceived latency on live pitch knob twists was ~35-
// 45 ms — past the 20-30 ms threshold where it stops feeling
// real-time.  Phase-vocoder algorithms reach ~12 ms latency without
// audible compromise (Laroche & Dolson 1999 — see PLAN_FX_RACK_V0.3.md
// for the discussion).
//
// Provenance
// ──────────
//   Upstream:  github.com/olvb/phaze @ master
//   File:      www/phase-vocoder.js (bundled via browserify; the
//              source is in phaze's src/phase-vocoder.js + src/
//              ola-processor.js + fft.js dependency).  Vendored as a
//              pre-bundled .js so we don't have to add browserify /
//              fft.js to Slurmify's frontend toolchain.
//   License:   The Unlicense (effectively public domain; fully
//              compatible with Slurmify's GPL-3.0).
//   API:       registers as "phase-vocoder-processor"; exposes one
//              AudioParam `pitchFactor` (multiplier — 1.0 = no shift;
//              2.0 = one octave up; 0.5 = one octave down).  Convert
//              semitones to pitchFactor with Math.pow(2, semitones/12).
//
// Burn-fx (offline) still uses pyrubberband on the Python side
// (slurmcore._fx_pitch) for higher-quality phase-vocoder rendering;
// this file is the LIVE PREVIEW engine only.
//
// To rebase the fork on a newer phaze release: re-clone the repo,
// run `npm install && npm run build-audio-worklet`, and replace the
// bundled content below.  Keep this header.
// ──────────────────────────────────────────────────────────────────────
